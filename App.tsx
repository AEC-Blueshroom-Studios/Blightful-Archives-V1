...
const BOSS_BEHAVIORS: Record<number, Function> = {
    ...
    1: createPhasedAI([{ name: 'ORBITAL', duration: 200, interval: 5, action: (b:any,p:any,t:any) => { let a = t*0.2; spawnBullet(b.x + Math.cos(a)*100, b.y + Math.sin(a)*100, Math.cos(a)*5, Math.sin(a)*5, "#e67e22"); } }, { name: 'FLARE', duration: 150, interval: 60, action: (b:any,p:any) => { for(let i=0; i<20; i++) { let a = (i/20)*Math.PI*2; spawnBullet(b.x, b.y, Math.cos(a)*8, Math.sin(a)*8, "#d35400"); } } }, { name: 'CINDER_FALL', duration: 180, interval: 10, action: (b:any,p:any) => { spawnBullet(Math.random()*800, 0, 0, 5, "#e67e22", {gravity: 0.1}); } }, { name: 'CORE_DUMP', duration: 120, interval: 3, action: (b:any,p:any) => { spawnBullet(b.x, b.y, (Math.random()-0.5)*15, 8, "#fff"); } }]),
    2: createPhasedAI([{ name: 'PRISM', duration: 180, interval: 30, action: (b:any,p:any) => { for(let i=0; i<5; i++) { b.activeLaser = i; spawnBullet(b.x - 40 + i*20, b.y, 0, 8, "#ff7675"); } }, move: 'center' }, { name: 'REFRACT', duration: 150, interval: 10, action: (b:any,p:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, 5, "#fff") }, { name: 'SHARD_SPRAY', duration: 120, interval: 5, action: (b:any,p:any) => { spawnBullet(b.x, b.y, (Math.random()-0.5)*15, (Math.random()-0.5)*15, "#ff7675"); } }, { name: 'LIGHT_WALL', duration: 160, interval: 40, action: (b:any,p:any) => { for(let i=0;i<10;i++) spawnBullet(i*80, 0, 0, 5, "#fff"); } }]),
    3: createPhasedAI([{ name: 'SWEEP', duration: 200, interval: 5, action: (b:any,p:any,t:any) => { b.handAngle = t*0.05; spawnBullet(b.x, b.y, Math.cos(b.handAngle)*10, Math.sin(b.handAngle)*10, "#d4af37"); } }, { name: 'DILATE', duration: 150, interval: 30, action: (b:any,p:any) => { spawnBullet(Math.random()*800, 0, 0, 5, "#fff"); } }, { name: 'CHRONO_BURST', duration: 100, interval: 20, action: (b:any,p:any) => { for(let i=0; i<12; i++) spawnBullet(b.x, b.y, Math.cos(i*0.5)*10, Math.sin(i*0.5)*10, "#d4af37"); } }, { name: 'PENDULUM', duration: 180, interval: 5, action: (b:any,p:any,t:any) => { spawnBullet(b.x + Math.sin(t*0.1)*100, b.y, 0, 8, "#fff"); } }]),
    4: createPhasedAI([{ name: 'COLLAPSE', duration: 180, interval: 20, action: (b:any,p:any) => { for(let i=0; i<8; i++) { let a = (i/8)*Math.PI*2; spawnBullet(b.x + Math.cos(a)*200, b.y + Math.sin(a)*200, -Math.cos(a)*3, -Math.sin(a)*3, "#9b59b6"); } } }, { name: 'GRAVITY_WELL', duration: 200, interval: 5, action: (b:any,p:any) => { /* Logic in update */ } }, { name: 'EVENT_HORIZON', duration: 150, interval: 10, action: (b:any,p:any) => { spawnBullet(0, Math.random()*600, 10, 0, "#9b59b6"); spawnBullet(800, Math.random()*600, -10, 0, "#9b59b6"); } }, { name: 'SINGULARITY_SHOT', duration: 120, interval: 40, action: (b:any,p:any) => { spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#fff", {size: 20}); } }]),
    5: createPhasedAI([
        { name: 'PISTON_CRUSH', duration: 120, interval: 40, action: (b:any) => { spawnBullet(b.x-50, b.y, 0, 8, "#fff"); spawnBullet(b.x+50, b.y, 0, 8, "#fff"); } },
        { name: 'HEAT_WAVE', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, 5, b.col) },
        { name: 'OVERHEAT', duration: 180, interval: 5, action: (b:any, p:any) => spawnBullet(b.x, b.y, Math.cos(Math.atan2(p.y-b.y, p.x-b.x))*6, Math.sin(Math.atan2(p.y-b.y, p.x-b.x))*6, b.col) },
        { name: 'VENT', duration: 140, interval: 20, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i)*5, Math.sin(i)*5, "#fff"); } }
    ]),
    6: createPhasedAI([
        { name: 'BLUEPRINT', duration: 200, interval: 50, action: (b:any) => { for(let i=-2;i<=2;i++) spawnBullet(b.x+i*40, b.y, 0, 4, b.col); } },
        { name: 'CONSTRUCT', duration: 150, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.02, (p.y-b.y)*0.02, "#fff") },
        { name: 'DECONSTRUCT', duration: 120, interval: 10, action: (b:any) => spawnBullet(b.x + (Math.random()-0.5)*100, b.y, 0, 6, b.col) },
        { name: 'TOWER', duration: 160, interval: 60, action: (b:any) => { spawnBullet(100,0,0,5,b.col,{size:20}); spawnBullet(700,0,0,5,b.col,{size:20}); } }
    ]),
    7: createPhasedAI([
        { name: 'ARMOR_UP', duration: 180, interval: 5, action: (b:any,p:any,t:number) => spawnBullet(b.x+Math.cos(t*0.2)*50, b.y+Math.sin(t*0.2)*50, 0, 0, b.col) },
        { name: 'HEAVY_SLAM', duration: 100, interval: 40, action: (b:any) => { for(let i=0;i<5;i++) spawnBullet(b.x+(i-2)*30, b.y, 0, 10, "#fff"); } },
        { name: 'SHIELD_BASH', duration: 140, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, b.col, {size: 15}) },
        { name: 'METAL_RAIN', duration: 160, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 8, "#555") }
    ]),
    8: createPhasedAI([
        { name: 'GOLD_RUSH', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*15, 4, "#ffd700") },
        { name: 'MIDAS_TOUCH', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#fff") },
        { name: 'COIN_TOSS', duration: 140, interval: 20, action: (b:any) => { spawnBullet(b.x, b.y, -5, -5, "#ffd700"); spawnBullet(b.x, b.y, 5, -5, "#ffd700"); } },
        { name: 'WEALTH', duration: 180, interval: 50, action: (b:any) => { for(let i=0;i<12;i++) spawnBullet(b.x, b.y, Math.cos(i*0.5)*6, Math.sin(i*0.5)*6, "#fff"); } }
    ]),
    9: createPhasedAI([
        { name: 'INFECT', duration: 160, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.01 + (Math.random()-0.5)*2, (p.y-b.y)*0.01 + (Math.random()-0.5)*2, "#0f0") },
        { name: 'SPREAD', duration: 140, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(Math.random()*6.28)*5, Math.sin(Math.random()*6.28)*5, "#0f0") },
        { name: 'SPORE', duration: 120, interval: 40, action: (b:any) => spawnBullet(b.x, b.y, 0, 2, "#fff", {size: 10, split: true}) },
        { name: 'PLAGUE', duration: 180, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 4, "#0f0") }
    ]),
    10: createPhasedAI([
        { name: 'SHARD', duration: 150, interval: 15, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*8, 6, "#0ff") },
        { name: 'REFLECT', duration: 120, interval: 30, action: (b:any) => { spawnBullet(0, Math.random()*600, 5, 0, "#fff"); spawnBullet(800, Math.random()*600, -5, 0, "#fff"); } },
        { name: 'PRISM_BEAM', duration: 140, interval: 2, action: (b:any) => spawnBullet(b.x, b.y, Math.sin(b.timer*0.1)*5, 8, "#0ff") },
        { name: 'CLARITY', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i)*8, Math.sin(i)*8, "#fff"); } }
    ]),
    11: createPhasedAI([
        { name: 'HEAVY_HIT', duration: 120, interval: 50, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.08, (p.y-b.y)*0.08, "#555", {size: 15}) },
        { name: 'DENSITY', duration: 180, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*5, 3, "#333") },
        { name: 'WALL', duration: 150, interval: 40, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(i*80, 0, 0, 2, "#555"); } },
        { name: 'CRUSH', duration: 140, interval: 30, action: (b:any) => spawnBullet(b.x, b.y, 0, 15, "#fff") }
    ]),
    12: createPhasedAI([
        { name: 'SUN_SPOT', duration: 150, interval: 20, action: (b:any,p:any) => spawnBullet(p.x, 0, 0, 5, "#f90") },
        { name: 'FLARE', duration: 120, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.2)*6, Math.sin(b.timer*0.2)*6, "#f00") },
        { name: 'HEAT', duration: 140, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, (Math.random()-0.5)*10, "#f90") },
        { name: 'BURN', duration: 160, interval: 50, action: (b:any) => { for(let i=0;i<16;i++) spawnBullet(b.x, b.y, Math.cos(i*0.4)*4, Math.sin(i*0.4)*4, "#fff"); } }
    ]),
    13: createPhasedAI([
        { name: 'PING', duration: 120, interval: 40, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#0f0") },
        { name: 'SCAN', duration: 180, interval: 2, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.05)*8, Math.sin(b.timer*0.05)*8, "#0f0") },
        { name: 'TRACK', duration: 140, interval: 15, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.02, (p.y-b.y)*0.02, "#fff") },
        { name: 'LOCK', duration: 150, interval: 60, action: (b:any,p:any) => { for(let i=0;i<4;i++) spawnBullet(p.x+(i-1.5)*30, 0, 0, 10, "#f00"); } }
    ]),
    14: createPhasedAI([
        { name: 'GRIND', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer)*5, Math.sin(b.timer)*5, "#888") },
        { name: 'COG', duration: 120, interval: 40, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i*0.8)*4, Math.sin(i*0.8)*4, "#fff"); } },
        { name: 'JAM', duration: 140, interval: 20, action: (b:any) => spawnBullet(Math.random()*800, Math.random()*200, 0, 3, "#888") },
        { name: 'CRUSH', duration: 160, interval: 50, action: (b:any) => { spawnBullet(0, 300, 5, 0, "#888", {size:20}); spawnBullet(800, 300, -5, 0, "#888", {size:20}); } }
    ]),
    15: createPhasedAI([
        { name: 'FLUTTER', duration: 140, interval: 15, action: (b:any) => spawnBullet(b.x, b.y, Math.sin(b.timer*0.1)*5, 5, "#f0f") },
        { name: 'WING_BEAT', duration: 120, interval: 30, action: (b:any) => { spawnBullet(b.x-20, b.y, -3, 5, "#fff"); spawnBullet(b.x+20, b.y, 3, 5, "#fff"); } },
        { name: 'POLLEN', duration: 160, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*8, (Math.random()-0.5)*8, "#f0f") },
        { name: 'STORM', duration: 150, interval: 40, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(b.x, b.y, Math.cos(i)*6, Math.sin(i)*6, "#fff"); } }
    ]),
    16: createPhasedAI([
        { name: 'HELIX', duration: 180, interval: 5, action: (b:any,p:any,t:number) => spawnBullet(b.x+Math.cos(t*0.1)*30, b.y, 0, 5, "#00f") },
        { name: 'SPLIT', duration: 120, interval: 30, action: (b:any) => { spawnBullet(b.x, b.y, -4, 4, "#fff"); spawnBullet(b.x, b.y, 4, 4, "#fff"); } },
        { name: 'REPLICATE', duration: 140, interval: 10, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.03, (p.y-b.y)*0.03, "#00f") },
        { name: 'EVOLVE', duration: 150, interval: 50, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i)*8, Math.sin(i)*8, "#fff"); } }
    ]),
    17: createPhasedAI([
        { name: 'ROYAL_GUARD', duration: 160, interval: 20, action: (b:any) => { for(let i=0;i<3;i++) spawnBullet(b.x+(i-1)*40, b.y+40, 0, 5, "#50f"); } },
        { name: 'SCEPTRE', duration: 120, interval: 40, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#fff", {size: 10}) },
        { name: 'COMMAND', duration: 140, interval: 10, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 6, "#50f") },
        { name: 'EXECUTE', duration: 150, interval: 60, action: (b:any,p:any) => spawnBullet(p.x, 0, 0, 15, "#fff") }
    ]),
    18: createPhasedAI([
        { name: 'CLOUD', duration: 180, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, (Math.random()-0.5)*10, "#0ff") },
        { name: 'STAR_BIRTH', duration: 120, interval: 40, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i*0.8)*5, Math.sin(i*0.8)*5, "#fff"); } },
        { name: 'GAS', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x+Math.cos(b.timer*0.1)*50, b.y+Math.sin(b.timer*0.1)*50, 0, 0, "#0ff") },
        { name: 'SUPERNOVA', duration: 140, interval: 60, action: (b:any) => { for(let i=0;i<20;i++) spawnBullet(b.x, b.y, Math.cos(i*0.3)*8, Math.sin(i*0.3)*8, "#fff"); } }
    ]),
    19: createPhasedAI([
        { name: 'ZAP', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.1, (p.y-b.y)*0.1, "#ff0") },
        { name: 'STATIC', duration: 160, interval: 5, action: (b:any) => spawnBullet(b.x + (Math.random()-0.5)*100, b.y + (Math.random()-0.5)*100, 0, 0, "#ff0") },
        { name: 'CONDUCT', duration: 140, interval: 20, action: (b:any) => { spawnBullet(b.x, b.y, -5, 5, "#fff"); spawnBullet(b.x, b.y, 5, 5, "#fff"); } },
        { name: 'STORM', duration: 150, interval: 10, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 10, "#ff0") }
    ]),
    20: createPhasedAI([
        { name: 'ERROR', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*15, (Math.random()-0.5)*15, "#f00") },
        { name: 'ARTIFACT', duration: 120, interval: 40, action: (b:any) => { for(let i=0;i<4;i++) spawnBullet(b.x, b.y, Math.cos(i*1.5)*6, Math.sin(i*1.5)*6, "#fff"); } },
        { name: 'LAG', duration: 140, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.01, (p.y-b.y)*0.01, "#f00", {size: 15}) },
        { name: 'CRASH', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<30;i++) spawnBullet(b.x, b.y, Math.cos(i)*10, Math.sin(i)*10, "#fff"); } }
    ]),
    21: createPhasedAI([
        { name: 'ANCHOR', duration: 180, interval: 50, action: (b:any,p:any) => spawnBullet(p.x, 0, 0, 15, "#0f0") },
        { name: 'WEIGHT', duration: 120, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, 0, 8, "#fff") },
        { name: 'SINK', duration: 140, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#0f0") },
        { name: 'DROP', duration: 150, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 10, "#0f0") }
    ]),
    22: createPhasedAI([
        { name: 'BINARY', duration: 120, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.random()>0.5?5:-5, 5, "#0ff") },
        { name: 'STREAM', duration: 160, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, 0, 8, "#fff") },
        { name: 'FALL', duration: 140, interval: 20, action: (b:any) => { for(let i=0;i<5;i++) spawnBullet(b.x+(i-2)*20, b.y, 0, 6, "#0ff"); } },
        { name: 'MATRIX', duration: 150, interval: 40, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(i*80, 0, 0, 5, "#fff"); } }
    ]),
    23: createPhasedAI([
        { name: 'RING', duration: 150, interval: 50, action: (b:any) => { for(let i=0;i<12;i++) spawnBullet(b.x, b.y, Math.cos(i*0.5)*4, Math.sin(i*0.5)*4, "#555"); } },
        { name: 'ORBIT', duration: 180, interval: 5, action: (b:any,p:any,t:number) => spawnBullet(b.x+Math.cos(t*0.1)*60, b.y+Math.sin(t*0.1)*60, 0, 0, "#fff") },
        { name: 'DEBRIS', duration: 120, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*8, 4, "#555") },
        { name: 'GRAVITY', duration: 140, interval: 40, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#fff", {size: 10}) }
    ]),
    24: createPhasedAI([
        { name: 'CAGE', duration: 160, interval: 40, action: (b:any,p:any) => { for(let i=0;i<8;i++) spawnBullet(p.x+Math.cos(i*0.8)*60, p.y+Math.sin(i*0.8)*60, 0, 0, "#f90"); } },
        { name: 'TRAP', duration: 120, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.06, (p.y-b.y)*0.06, "#fff") },
        { name: 'CLOSE', duration: 140, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.2)*5, Math.sin(b.timer*0.2)*5, "#f90") },
        { name: 'LOCKDOWN', duration: 150, interval: 60, action: (b:any) => { for(let i=0;i<20;i++) spawnBullet(b.x, b.y, Math.cos(i*0.3)*7, Math.sin(i*0.3)*7, "#fff"); } }
    ]),
    25: createPhasedAI([
        { name: 'ERUPT', duration: 140, interval: 20, action: (b:any) => { spawnBullet(b.x, b.y, -3, -5, "#f00"); spawnBullet(b.x, b.y, 3, -5, "#f00"); } },
        { name: 'FLOW', duration: 160, interval: 5, action: (b:any) => spawnBullet(b.x + (Math.random()-0.5)*20, b.y, 0, 6, "#f00") },
        { name: 'ASH', duration: 120, interval: 10, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 3, "#555") },
        { name: 'VOLCANO', duration: 150, interval: 50, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(b.x, b.y, Math.cos(i*0.6)*8, Math.sin(i*0.6)*8, "#fff"); } }
    ]),
    26: createPhasedAI([
        { name: 'SLICE', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.08, (p.y-b.y)*0.08, "#ccc", {size: 3, pierce: 1}) },
        { name: 'REAP', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.1)*6, Math.sin(b.timer*0.1)*6, "#ccc") },
        { name: 'SHINE', duration: 140, interval: 40, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i)*5, Math.sin(i)*5, "#fff"); } },
        { name: 'EXECUTE', duration: 160, interval: 60, action: (b:any,p:any) => spawnBullet(p.x, 0, 0, 20, "#ccc", {size: 5}) }
    ]),
    27: createPhasedAI([
        { name: 'CURSE', duration: 150, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#505") },
        { name: 'HEX', duration: 120, interval: 40, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i)*5, Math.sin(i)*5, "#fff"); } },
        { name: 'RITUAL', duration: 160, interval: 5, action: (b:any) => spawnBullet(b.x+Math.cos(b.timer*0.2)*40, b.y+Math.sin(b.timer*0.2)*40, 0, 0, "#505") },
        { name: 'DOOM', duration: 140, interval: 50, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.08, (p.y-b.y)*0.08, "#fff", {size: 15}) }
    ]),
    28: createPhasedAI([
        { name: 'BLOCK', duration: 180, interval: 5, action: (b:any) => spawnBullet(b.x + (Math.random()-0.5)*60, b.y + 40, 0, 2, "#0f8") },
        { name: 'PILLAR', duration: 120, interval: 40, action: (b:any) => { spawnBullet(b.x-40, b.y, 0, 8, "#fff"); spawnBullet(b.x+40, b.y, 0, 8, "#fff"); } },
        { name: 'REFRESH', duration: 150, interval: 20, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*8, (Math.random()-0.5)*8, "#0f8") },
        { name: 'MONOLITH', duration: 140, interval: 60, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(i*80, 0, 0, 6, "#fff"); } }
    ]),
    29: createPhasedAI([
        { name: 'SWING', duration: 140, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.15)*7, Math.sin(b.timer*0.15)*7, "#f90") },
        { name: 'TICK', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#fff") },
        { name: 'TOCK', duration: 150, interval: 40, action: (b:any) => { for(let i=0;i<4;i++) spawnBullet(b.x, b.y, Math.cos(i*1.5)*6, Math.sin(i*1.5)*6, "#f90"); } },
        { name: 'ALARM', duration: 160, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 8, "#fff") }
    ]),
    30: createPhasedAI([
        { name: 'ZERO', duration: 120, interval: 20, action: (b:any) => { for(let i=0;i<4;i++) spawnBullet(b.x, b.y, Math.cos(i*1.5)*5, Math.sin(i*1.5)*5, "#fff"); } },
        { name: 'VOID', duration: 150, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, (Math.random()-0.5)*10, "#000") },
        { name: 'NULL', duration: 140, interval: 40, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.06, (p.y-b.y)*0.06, "#fff", {size: 10}) },
        { name: 'DELETE', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<20;i++) spawnBullet(b.x, b.y, Math.cos(i*0.3)*8, Math.sin(i*0.3)*8, "#000"); } }
    ]),
    31: createPhasedAI([
        { name: 'STREAK', duration: 140, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, 0, 10, "#f00") },
        { name: 'COMET', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.07, (p.y-b.y)*0.07, "#fff") },
        { name: 'IMPACT', duration: 150, interval: 50, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i*0.8)*6, Math.sin(i*0.8)*6, "#f00"); } },
        { name: 'CRASH', duration: 160, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 12, "#fff") }
    ]),
    32: createPhasedAI([
        { name: 'SWIRL', duration: 150, interval: 5, action: (b:any,p:any,t:number) => spawnBullet(b.x, b.y, Math.cos(t*0.2)*5, Math.sin(t*0.2)*5, "#60f") },
        { name: 'SUCK', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#fff") },
        { name: 'SPIN', duration: 140, interval: 40, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i*0.6)*6, Math.sin(i*0.6)*6, "#60f"); } },
        { name: 'VOID', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<16;i++) spawnBullet(b.x, b.y, Math.cos(i*0.4)*7, Math.sin(i*0.4)*7, "#fff"); } }
    ]),
    33: createPhasedAI([
        { name: 'GAZE', duration: 120, interval: 10, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.03, (p.y-b.y)*0.03, "#0f0") },
        { name: 'BLINK', duration: 150, interval: 40, action: (b:any) => { for(let i=0;i<5;i++) spawnBullet(b.x, b.y, Math.cos(i*1.2)*5, Math.sin(i*1.2)*5, "#fff"); } },
        { name: 'STARE', duration: 140, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*5, 8, "#0f0") },
        { name: 'LASER', duration: 160, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.1, (p.y-b.y)*0.1, "#fff", {size: 8}) }
    ]),
    34: createPhasedAI([
        { name: 'SLITHER', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.sin(b.timer*0.1)*5, 5, "#fc0") },
        { name: 'BITE', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.06, (p.y-b.y)*0.06, "#fff") },
        { name: 'RATTLE', duration: 140, interval: 5, action: (b:any) => spawnBullet(b.x + (Math.random()-0.5)*20, b.y, 0, 7, "#fc0") },
        { name: 'VENOM', duration: 160, interval: 50, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i*0.6)*6, Math.sin(i*0.6)*6, "#fff"); } }
    ]),
    35: createPhasedAI([
        { name: 'DARKNESS', duration: 180, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, (Math.random()-0.5)*10, "#333") },
        { name: 'SHADOW', duration: 120, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#000") },
        { name: 'ECLIPSE', duration: 140, interval: 40, action: (b:any) => { for(let i=0;i<12;i++) spawnBullet(b.x, b.y, Math.cos(i*0.5)*5, Math.sin(i*0.5)*5, "#fff"); } },
        { name: 'NIGHT', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<20;i++) spawnBullet(b.x, b.y, Math.cos(i*0.3)*6, Math.sin(i*0.3)*6, "#333"); } }
    ]),
    36: createPhasedAI([
        { name: 'FAN', duration: 150, interval: 15, action: (b:any) => { for(let i=0;i<3;i++) spawnBullet(b.x, b.y, Math.cos(2+i*0.5)*6, Math.sin(2+i*0.5)*6, "#f66"); } },
        { name: 'HEAT', duration: 120, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*8, 5, "#fff") },
        { name: 'WAVE', duration: 140, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#f66") },
        { name: 'INFERNO', duration: 160, interval: 50, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(b.x, b.y, Math.cos(i)*7, Math.sin(i)*7, "#fff"); } }
    ]),
    37: createPhasedAI([
        { name: 'FREEZE', duration: 140, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.03, (p.y-b.y)*0.03, "#0ff") },
        { name: 'SHARD', duration: 120, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*6, 6, "#fff") },
        { name: 'ICICLE', duration: 150, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 8, "#0ff") },
        { name: 'BLIZZARD', duration: 160, interval: 40, action: (b:any) => { for(let i=0;i<12;i++) spawnBullet(b.x, b.y, Math.cos(i*0.5)*6, Math.sin(i*0.5)*6, "#fff"); } }
    ]),
    38: createPhasedAI([
        { name: 'BOLT', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.08, (p.y-b.y)*0.08, "#ff0") },
        { name: 'SPARK', duration: 150, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, (Math.random()-0.5)*10, "#fff") },
        { name: 'THUNDER', duration: 140, interval: 40, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i)*7, Math.sin(i)*7, "#ff0"); } },
        { name: 'STORM', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<16;i++) spawnBullet(b.x, b.y, Math.cos(i*0.4)*6, Math.sin(i*0.4)*6, "#fff"); } }
    ]),
    39: createPhasedAI([
        { name: 'FADE', duration: 140, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#ccc", {size: 3}) },
        { name: 'GHOST', duration: 120, interval: 10, action: (b:any) => spawnBullet(b.x + (Math.random()-0.5)*50, b.y + (Math.random()-0.5)*50, 0, 0, "#fff") },
        { name: 'SPOOK', duration: 150, interval: 30, action: (b:any) => { spawnBullet(b.x-30, b.y, 0, 5, "#ccc"); spawnBullet(b.x+30, b.y, 0, 5, "#ccc"); } },
        { name: 'HAUNT', duration: 160, interval: 50, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i*0.8)*6, Math.sin(i*0.8)*6, "#fff"); } }
    ]),
    40: createPhasedAI([
        { name: 'GROWTH', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.2)*6, Math.sin(b.timer*0.2)*6, "#0f0") },
        { name: 'VINE', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#fff") },
        { name: 'THORN', duration: 140, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*8, 5, "#0f0") },
        { name: 'JUNGLE', duration: 160, interval: 40, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(b.x, b.y, Math.cos(i)*7, Math.sin(i)*7, "#fff"); } }
    ]),
    41: createPhasedAI([
        { name: 'ATOM', duration: 140, interval: 15, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.3)*5, Math.sin(b.timer*0.3)*5, "#f90") },
        { name: 'SPLIT', duration: 120, interval: 40, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i)*6, Math.sin(i)*6, "#fff"); } },
        { name: 'FUSION', duration: 150, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.06, (p.y-b.y)*0.06, "#f90") },
        { name: 'NUKE', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<20;i++) spawnBullet(b.x, b.y, Math.cos(i*0.3)*8, Math.sin(i*0.3)*8, "#fff"); } }
    ]),
    42: createPhasedAI([
        { name: 'BEAM', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.1, (p.y-b.y)*0.1, "#f90", {size: 5}) },
        { name: 'GLOW', duration: 150, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*10, (Math.random()-0.5)*10, "#fff") },
        { name: 'FLASH', duration: 140, interval: 40, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i*0.8)*6, Math.sin(i*0.8)*6, "#f90"); } },
        { name: 'BLIND', duration: 160, interval: 50, action: (b:any) => { for(let i=0;i<12;i++) spawnBullet(b.x, b.y, Math.cos(i*0.5)*7, Math.sin(i*0.5)*7, "#fff"); } }
    ]),
    43: createPhasedAI([
        { name: 'ECHO', duration: 140, interval: 20, action: (b:any,p:any) => { spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#00f"); spawnBullet(b.x, b.y, (p.x-b.x)*0.03, (p.y-b.y)*0.03, "#00f"); } },
        { name: 'REVERB', duration: 120, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.cos(b.timer*0.1)*6, 6, "#fff") },
        { name: 'SONIC', duration: 150, interval: 30, action: (b:any) => { for(let i=0;i<4;i++) spawnBullet(b.x, b.y, Math.cos(i*1.5)*5, Math.sin(i*1.5)*5, "#00f"); } },
        { name: 'NOISE', duration: 160, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 10, "#fff") }
    ]),
    44: createPhasedAI([
        { name: 'BONE', duration: 150, interval: 20, action: (b:any) => { spawnBullet(b.x-20, b.y, 0, 6, "#fff"); spawnBullet(b.x+20, b.y, 0, 6, "#fff"); } },
        { name: 'SKULL', duration: 120, interval: 40, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#ccc", {size: 10}) },
        { name: 'RIB', duration: 140, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*8, 5, "#fff") },
        { name: 'GRAVE', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<10;i++) spawnBullet(b.x, b.y, Math.cos(i)*7, Math.sin(i)*7, "#ccc"); } }
    ]),
    45: createPhasedAI([
        { name: 'RAIN', duration: 140, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 12, "#00f") },
        { name: 'WIND', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.06, (p.y-b.y)*0.06, "#fff") },
        { name: 'THUNDER', duration: 150, interval: 40, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i)*7, Math.sin(i)*7, "#00f"); } },
        { name: 'FLOOD', duration: 160, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*15, 6, "#fff") }
    ]),
    46: createPhasedAI([
        { name: 'PIXEL', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, Math.round((Math.random()-0.5)*10), Math.round((Math.random()-0.5)*10), "#0f0") },
        { name: 'BLOCK', duration: 120, interval: 30, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.04, (p.y-b.y)*0.04, "#fff", {size: 8}) },
        { name: 'LINE', duration: 140, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, 0, 10, "#0f0") },
        { name: 'GRID', duration: 160, interval: 50, action: (b:any) => { for(let i=0;i<12;i++) spawnBullet(b.x, b.y, Math.cos(i*0.5)*6, Math.sin(i*0.5)*6, "#fff"); } }
    ]),
    47: createPhasedAI([
        { name: 'SPOOK', duration: 140, interval: 20, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.05, (p.y-b.y)*0.05, "#ccc") },
        { name: 'GEAR', duration: 120, interval: 40, action: (b:any) => { for(let i=0;i<8;i++) spawnBullet(b.x, b.y, Math.cos(i*0.8)*5, Math.sin(i*0.8)*5, "#fff"); } },
        { name: 'MACHINE', duration: 150, interval: 10, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*6, 6, "#ccc") },
        { name: 'SOUL', duration: 160, interval: 5, action: (b:any) => spawnBullet(Math.random()*800, 0, 0, 8, "#fff") }
    ]),
    48: createPhasedAI([
        { name: 'CORRUPT', duration: 150, interval: 10, action: (b:any,p:any) => spawnBullet(b.x, b.y, (p.x-b.x)*0.03 + (Math.random()-0.5)*2, (p.y-b.y)*0.03 + (Math.random()-0.5)*2, "#f00") },
        { name: 'ERROR', duration: 120, interval: 30, action: (b:any) => { for(let i=0;i<6;i++) spawnBullet(b.x, b.y, Math.cos(i)*6, Math.sin(i)*6, "#fff"); } },
        { name: 'GLITCH', duration: 140, interval: 5, action: (b:any) => spawnBullet(b.x, b.y, (Math.random()-0.5)*12, (Math.random()-0.5)*12, "#f00") },
        { name: 'FAIL', duration: 160, interval: 60, action: (b:any) => { for(let i=0;i<20;i++) spawnBullet(b.x, b.y, Math.cos(i*0.3)*8, Math.sin(i*0.3)*8, "#fff"); } }
    ]),
    49: createPhasedAI([
